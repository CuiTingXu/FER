...
def PublicTest(epoch):
    ...
    if PublicTest_acc > best_PublicTest_acc:
        ...
        if not os.path.isdir(path):
            os.mkdir(path)
        torch.save(state, os.path.join(path, 'Fer2013_premodel_VGG19_Public_6lass_state_edition1_github.t7'))
...
def PrivateTest(epoch):
    ...
    early_stopping(PrivateTest_loss/ (batch_idx + 1),net)
    ...
    if PrivateTest_acc > best_PrivateTest_acc:
        ...
        if not os.path.isdir(path):
            os.mkdir(path)
        torch.save(state, os.path.join(path, 'Fer2013_premodel_VGG19_Private_6class_state_edition1_github.t7'))
        best_PrivateTest_acc = PrivateTest_acc
        best_PrivateTest_acc_epoch = epoch
